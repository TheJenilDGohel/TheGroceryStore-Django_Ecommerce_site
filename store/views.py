...
def contact(request):
    if request.method == 'POST':
        ...
        data = Contact(name = Name,email = email,phone = phone,message = message,timestamp = current_datetime)
        data.save()
...
class Search(View):
    ...
    def post(self,request):
        ...
        products = []
        if query_name:
            products = Product.objects.filter(name__icontains = query_name).order_by('-product_id')
